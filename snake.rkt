...
;   The Snake game
;   Utilize a função (start-snake) para começar o jogo
;   Utilizando as teclas de setas colete os barris tóxicos

(require 2htdp/image 2htdp/universe)
(require (lib "graphics.ss" "graphics"))
(open-graphics);
(require (prefix-in base: racket/gui/base))
;; -----------------------------------------------------------------------------
;; Data Definitions
...
;; Represents the ranking from the player with some nick and score
(struct ranking (score nick))

;;------------------------------------------------------------------------------
; Variáveis do player
(define name "Player")
(define (save-name inputName)
  (cond [(non-empty-string? inputName) (set! name inputName)]))

; Tela de adicionar player
  (define (get-player)
    (define beginning (instantiate base:dialog% ("Snake")))
    (define player (new base:text-field% [parent beginning] [label "Nick:"]))
    (define panel (new base:horizontal-panel% [parent beginning]
                                     [alignment '(center center)]))
    (new base:button% [parent panel] [label "Confirmar"]
         [callback (lambda (button event)  (send beginning show #f) (save-name (send player get-value)) (sleep 1))])
    (send beginning show #t))

(get-player)

(define score-counter 0)

;;------------------------------------------------------------------------------
; FILE
(define rank (open-output-file "rank.txt" #:mode 'text #:exists 'append))

(define (get-rank position)
    (let
        ([lst1(file->list "rank.txt")])
        (define lst2 (build-list (length lst1) (lambda (x) 
           (define aux (string-split (list-ref (list-ref lst1 x) 0)) )
            (ranking (string->number (list-ref aux 0)) (list-ref aux 1) )
        )))
        (cond
          [(< position (length lst1))
           (string-append (number->string (ranking-score (list-ref (sort lst2 > #:key ranking-score) position))) " " (ranking-nick (list-ref (sort lst2 > #:key ranking-score) position)))]
          [else (string-append " " " ")]
        )
    )
)
...
;; GRAPHICAL BOARD
(define WIDTH-PX  (* SEG-SIZE 42))
(define HEIGHT-PX (* SEG-SIZE 30))
(define WIDTH-PLAYABLE (+(* SEG-SIZE 30) 1))
(define MIDDLE-Y (/ HEIGHT-PX 2))
(define ONE-QUARTER-Y (/ MIDDLE-Y 2))

;; Text Constants
(define TEXT-SIZE 20)
(define ENDGAME-TEXT-SIZE 20)
(define RECORDS-SCORE
  (text "Top Scores" TEXT-SIZE "black"))
(define CURRENT-PLAYER
  (text name TEXT-SIZE "black"))

;; Visual constants
(define MT-SCENE-1
  (place-image/align RECORDS-SCORE (* SEG-SIZE 38) 0 "right" "top"
  (place-image/align (text (get-rank 0) TEXT-SIZE "black") (* SEG-SIZE 38) 35 "right" "top"
  (place-image/align (text (get-rank 1) TEXT-SIZE "black") (* SEG-SIZE 38) 55 "right" "top"
  (place-image/align (text (get-rank 2) TEXT-SIZE "black") (* SEG-SIZE 38) 75 "right" "top"
  (place-image/align (text (get-rank 3) TEXT-SIZE "black") (* SEG-SIZE 38) 95 "right" "top"
  (place-image/align (text (get-rank 4) TEXT-SIZE "black") (* SEG-SIZE 38) 115 "right" "top"
  (place-image/align (text (get-rank 5) TEXT-SIZE "black") (* SEG-SIZE 38) 135 "right" "top"
  (place-image/align (text (get-rank 6) TEXT-SIZE "black") (* SEG-SIZE 38) 155 "right" "top"
  (place-image/align (text (get-rank 7) TEXT-SIZE "black") (* SEG-SIZE 38) 175 "right" "top"
  (place-image/align (text (get-rank 8) TEXT-SIZE "black") (* SEG-SIZE 38) 195 "right" "top"
  (place-image/align (text (get-rank 9) TEXT-SIZE "black") (* SEG-SIZE 38) 215 "right" "top"
  (empty-scene WIDTH-PX HEIGHT-PX)))))))))))))

(define MT-SCENE
  (add-line MT-SCENE-1
          WIDTH-PLAYABLE HEIGHT-PX WIDTH-PLAYABLE 0 "black"))
...
(define (render-end w)
  (write (list (string-append (number->string score-counter) " " name)) rank)
  (close-output-port rank)
  (overlay (text (string-append name ", seu score final é " (number->string score-counter)) TEXT-SIZE "blue")
           (render-pit w)))
...
(define (grow sn)
  (set! score-counter (+ score-counter 1))
